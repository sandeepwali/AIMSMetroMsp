...
@Slf4j
@Component
public class RequestingProductImageToMspTasklet implements Tasklet, InitializingBean {

	// /providerTransactionId/:providerTransactionId/requestType/link/countryCode/:countryCode/salesLine/:salesLine/storeNumber/:storeNumber
	...
	public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
		...
		requestBody.setWidth(message.getContent().getWidth());
		requestBody.setHeight(message.getContent().getHeight());
		requestBody.setColor(message.getContent().getColor());
		requestBody.setModel(message.getContent().getModel());
		requestBody.setType(message.getContent().getType());
